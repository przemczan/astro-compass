...
class AlignmentSolverTest {
    ...
    @Test
    fun resync_withNoDrift_preservesTheMountingCorrectionFromAThreeStarFit() {
        // A non-yaw rotation: only a full 3-DOF fit (not a yaw-only replace) can produce this,
        // so recovering it after resync proves the mounting correction survived.
        val mountingRotation = Quaternion.fromAxisAngle(Vector3(1.0, 0.5, 0.3), Angle.ofDegrees(22.0)).normalized()
        val existing = AlignmentSolver.solve(
            listOf(
                syntheticPoint(mountingRotation, sky(azDeg = 20.0, altDeg = 60.0), "star1"),
                syntheticPoint(mountingRotation, sky(azDeg = 150.0, altDeg = 35.0), "star2"),
                syntheticPoint(mountingRotation, sky(azDeg = 260.0, altDeg = 20.0), "star3"),
            ),
            nowEpochMillis = 0L,
        )
        assertIs<AlignmentResult.Success>(existing)

        val resyncPoint = syntheticPoint(mountingRotation, sky(azDeg = 300.0, altDeg = 55.0), "resync-target")
        val result = AlignmentSolver.resync(existing.model, resyncPoint, nowEpochMillis = 1_000L)

        assertIs<AlignmentResult.Success>(result)
        assertSameRotation(mountingRotation, result.model.sensorToSky)
    }

    @Test
    fun resync_correctsYawDrift_whileKeepingTheMountingCorrection() {
        val mountingRotation = Quaternion.fromAxisAngle(Vector3(1.0, 0.5, 0.3), Angle.ofDegrees(22.0)).normalized()
        val existing = AlignmentSolver.solve(
            listOf(
                syntheticPoint(mountingRotation, sky(azDeg = 20.0, altDeg = 60.0), "star1"),
        ...
        // Gyro yaw drift accumulated since the fit: the true sensor-to-sky rotation is now the
        // original mounting correction plus an additional rotation about the sky's up axis.
        val driftYaw = Quaternion.fromAxisAngle(Vector3.UNIT_Z, Angle.ofDegrees(-4.0))
        val driftedRotation = (driftYaw * mountingRotation).normalized()
        val resyncPoint = syntheticPoint(driftedRotation, sky(azDeg = 300.0, altDeg = 55.0), "resync-target")

        val result = AlignmentSolver.resync(existing.model, resyncPoint, nowEpochMillis = 1_000L)

        assertIs<AlignmentResult.Success>(result)
        assertSameRotation(driftedRotation, result.model.sensorToSky)
    }
}
