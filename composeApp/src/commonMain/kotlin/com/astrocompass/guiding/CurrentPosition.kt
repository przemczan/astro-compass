...
/** Where a catalog object is *right now*, of-date (JNow) -- what
 *  [com.astrocompass.telescope.TelescopeConnection.slewTo] expects. */
fun SkyObject.currentEquatorial(epochMillis: Long): EquatorialCoordinates {
    val julianCenturies = AstroTime.julianCenturiesJ2000(AstroTime.julianDay(epochMillis))
    return equatorialAt(julianCenturies)
}
...
fun SkyObject.currentHorizontal(location: ObserverLocation, epochMillis: Long): HorizontalCoordinates {
    ...
    return CoordinateTransforms.equatorialToHorizontal(currentEquatorial(epochMillis), lst, location.latitude)
}
