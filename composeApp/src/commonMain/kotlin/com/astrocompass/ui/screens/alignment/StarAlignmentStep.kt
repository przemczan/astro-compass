...
/**
 * The sensors-only branch of the alignment wizard: 2-3 star syncs, one star at a time -- pick a
 * star, point the telescope at it, confirm. The sky map is the permanent backdrop for all of that;
 * every step renders as an overlay on top of it rather than replacing it, so the surrounding sky
 * stays available for star-hopping by eye, and tapping a different star while one is already
...
fun StarAlignmentStep(
    ...
    /** Shared with the browse and guidance maps -- the same filter control and the same
     *  [MapObjectFilter.maxMagnitude]. Category toggles are inert here since [suggestStars] and the
     *  map's own catalog subset are already narrowed to [StarObject], which [MapObjectFilter.matches]
     *  never gates on category. */
    ...
    Scaffold(
        ...
        topBar = { TopAppBar(title = { Text(if (alignsMount) "Align telescope" else "Calibrate with stars") }) },
        bottomBar = {
            AppBottomBar(menu) {
                ToolbarActionButton(
                ...
                if (alignsMount) {
                    ...
                    if (session.mountAlignmentActive) {
            ...
            Column(Modifier.align(Alignment.BottomCenter).fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                when {
                    ...
                    phoneResult is AlignmentResult.Success -> AlignmentCompleteCard(
                        result = phoneResult,
                        onDone = { onSaveModel(phoneResult.model); session.clear(); onExit() },
                        modifier = stepModifier,
                    )
...
private fun AlignmentCompleteCard(result: AlignmentResult.Success, onDone: () -> Unit, modifier: Modifier = Modifier) {
    StepCard(modifier) {
        Row {
            ...
            Text(" Phone calibrated", style = MaterialTheme.typography.titleMedium)
        }
